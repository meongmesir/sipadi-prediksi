...
export function AdminPrediksi() {
  const [prediksiList, setPrediksiList] = useState<Prediksi[]>([]);
  ...
  useEffect(() => {
    fetchWithAuth('/admin/predictions')
      .then((data: any[]) => {
        const mapped: Prediksi[] = data.map((p: any) => ({
          id: p.id,
          petani: p.petani || `User #${p.user_id}`,
          provinsi: p.provinsi,
          luas: p.luas_lahan_ha,
          cultivar: p.cultivar_name,
          waterCode: p.water_code,
          sowingDoy: p.sowing_doy,
          nTotal: p.n_total_kg_ha,
          plantPop: p.plant_pop,
          yieldKgHa: p.yield_kg_ha,
          kategori: p.kategori,
          tanggal: p.created_at ? new Date(p.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }) : "-",
        }));
        setPrediksiList(mapped);
      })
      .catch((err) => console.error("Gagal memuat prediksi:", err))
  ...
  const rataHasil = prediksiList.length > 0
    ? Math.round(prediksiList.reduce((s, p) => s + p.yieldKgHa, 0) / prediksiList.length)
    : 0;

  if (loading) {
    return <div className="p-8 text-center text-gray-500">Memuat data prediksi...</div>;
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">{prediksiList.length} prediksi · rata-rata {rataHasil.toLocaleString("id-ID")} kg/ha</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Sangat Baik",    n: prediksiList.filter((p) => p.kategori === "Sangat Baik").length,    color: "bg-green-50 border-green-200 text-green-700",  sub: ">8.000 kg/ha" },
          { label: "Baik",           n: prediksiList.filter((p) => p.kategori === "Baik").length,           color: "bg-blue-50 border-blue-200 text-blue-700",     sub: "5.000–8.000 kg/ha" },
          { label: "Cukup",          n: prediksiList.filter((p) => p.kategori === "Cukup").length,          color: "bg-amber-50 border-amber-200 text-amber-700",  sub: "3.000–5.000 kg/ha" },
          { label: "Perlu Perhatian",n: prediksiList.filter((p) => p.kategori === "Perlu Perhatian").length,color: "bg-red-50 border-red-200 text-red-700",        sub: "<3.000 kg/ha" },
        ].map((c) => (
